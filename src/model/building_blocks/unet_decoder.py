...
class CoordHead(nn.Module):
    def __init__(self,
        ...
        """
        Coordinate head for coordinate regression. It assumes a PlainConvEncoder / ResidualEncoder. 
        """
        super().__init__()
        ...
        layers = [
            ...
            nn.Linear(coord_in_features, hidden_coord),
            nn.ReLU(inplace=True),
            nn.Linear(hidden_coord, hidden_coord),
            nn.ReLU(inplace=True),
            nn.Linear(hidden_coord, hidden_coord),
            nn.ReLU(inplace=True),
            nn.Linear(hidden_coord, K * 3),
        ]
    ...
    def forward(self, skips):
        ...
        :return:
        """
        x = skips[-1] # bottleneck: [B, C_bottleneck, d, h, w]
        # apply coordinate head on encoder output 
        coords = self.coord_head(x)
        coords = coords.view(coords.shape[0], self.K, 3)
        return coords
