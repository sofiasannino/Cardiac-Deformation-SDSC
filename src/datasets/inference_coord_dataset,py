...
class nnUNetDatasetCoordTest(nnUNetBaseDataset):
    def __init__(self, folder: str, identifiers: Optional[Dict[str, Dict[str, str]]] = None, labels : Tuple[int, ...] = (1, 2, 3), points_per_label : int = 1562,
                 max_num_patients: int = None, max_num_patients_test: int = None, num_frames_per_patient: int = 20):
        super().__init__(folder, identifiers)
        self.labels = labels # labels 
        self.K = points_per_label*3 # total number of control points
        if max_num_patients is not None and max_num_patients_test is not None:
            self.identifiers = dict(list(self.identifiers.items())[(max_num_patients*num_frames_per_patient):((max_num_patients_test+ max_num_patients)*num_frames_per_patient)])
        else : 
...
class nnUNetDataLoaderCoordTest(DataLoader):
    def __init__(self,
                 data: nnUNetDatasetCoordTest,
                 batch_size: int = 1,
